<?php
   ...
   if (isset($_POST['modifier-cat'])){
      ...
      $sql_update="UPDATE categorie SET nomcat='$NOMCAT' WHERE idcat=$idcat ";
      $result_update=$BDD->query($sql_update);
      if ($result_update){
            echo "<script>alert('catégorie modifiée avec succès!')</script>";
            echo "<script>window.open('index-admin.php?afficher_categories','_self')</script>";
        }
      }

    $idcat=$_GET['idcat'];
    $cat_select="SELECT * FROM categorie WHERE idcat=$idcat";
    $cat_return=$BDD->query($cat_select);
    $cat=$cat_return->fetch();
    $Nomcat=$cat['nomcat'];
?>
<!DOCTYPE html>
<html lang="en">
...
<form action="" method="POST" class="mb-2">
   <div class="input-group w-90 mb-2">
       ...
       <input type="text" class="form-control" name="categorie" placeholder="Nom de la catégorie" value="<?=$Nomcat?>">
    </div>
    <div class="input-group w-10 mb-2 m-auto">
      <input type="submit" class="p-2 my-3 border-0" name="modifier-cat" value="Ajouter nouvelle catégorie" style="background-color: #fde3e9;">
    </div>
</form>
